...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, action_std):
        ...
        self.actor = nn.Sequential(
            ...
            # nn.Linear(32, action_dim),
            NoisyLinear(32, action_dim, sigma_init=0.0017),
            nn.Tanh()
        )
        # critic
        self.critic = nn.Sequential(
            ...
            nn.Tanh(),
            # nn.Linear(64, 32),
            NoisyLinear(64, 32, sigma_init=0.0017),
            nn.Tanh(),
            nn.Linear(32, 1)
        )
...
def main():
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        if running_reward > (log_interval * solved_reward):
            print("########## Solved! ##########")
            torch.save(ppo.policy.state_dict(), './preTrained/PPO_continuous_solved_{}.pth'.format(env_name))
            # break

        # save every 500 episodes
        if i_episode % 500 == 0:
            torch.save(ppo.policy.state_dict(), './preTrained/PPO_continuous_{}_{}.pth'.format(env_name, i_episode))
